...
const CREATE_DRIVER_VEHICLE = gql`
...
const DriverNotAssignedVehicles = (props) => {
  ...
  const { loading, error, data, refetch } = useQuery(GET_DRIVER_VEHICLES, {
    variables: { 
  ...
  const [
    ...
    CREATE_DRIVER_VEHICLE, 
    {
      update: (store) => {
        ...
        const newData = dataNotAssigned.driverVehicles.data.filter(vehicle => !vehicleID.includes(vehicle.id));
        const newDataObj = {
          driverVehicles: {
            data: newData,
        ...
        try {
          ...
          const assigned = dataNotAssigned.driverVehicles.data.filter(vehicle => vehicleID.includes(vehicle.id));
          const newDataObj = {
            driverVehicles: {
              data: dataAssigned.driverVehicles.data.concat(assigned),
          ...
          console.log("No cache for assigned vehicles");
        }
      },
      onCompleted() {
        setVehicleID([])
  ...
  const hasData = () => data && data.driverVehicles.data && data.driverVehicles.data.length > 0
...
export default DriverNotAssignedVehicles