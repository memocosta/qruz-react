...
const EditVehicle = (props) => {
  const leo = props.vehicle.license_expires_on.split('-');
  const [licensePlate, setLicensePlate] = useState(props.vehicle.license_plate);
  const [carTypeID, setCarTypeID] = useState(props.vehicle.type.id);
  const [carModelID, setCarModelID] = useState(props.vehicle.model.id);
  const [carMakeID, setCarMakeID] = useState(props.vehicle.make.id);
  const [manufactureYear, setManufactureYear] = useState(props.vehicle.year);
  const [expDay, setExpDay] = useState(parseInt(leo[2]));
  const [expMonth, setExpMonth] = useState(parseInt(leo[1]));
  const [expYear, setExpYear] = useState(parseInt(leo[0]));
  const [file, setFile] = useState('');
  const photo = props.vehicle.photo || '';
  ...
  if (mutationError) {
    mutationFeedback = <p className='alert alert-danger mb-4 rounded-0'>Bummer! Oh no, we weren't able to save changes made on this vehicle. Please try again, if the issue persists, please shoot us an email at <em>support@qruz.app</em> and we'll help you out.</p>
  }
          ...
          <form encType="multipart/form-data"
            ...
            <div id="dialog-body" className="px-4 pt-4 pb-3">
              <div className="text-center mb-2">
                <Photo photo={photo} newPhoto={setFile} />
              </div>
